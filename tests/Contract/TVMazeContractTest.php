...
class TVMazeContractTest extends TestCase
{
    ...
    {
        // Using Bryan Cranston (person ID 489)
        $person = $this->client->getPerson(489);
        
        $this->assertInstanceOf(Person::class, $person);
        $this->assertEquals(489, $person->id);
        $this->assertStringContainsString('Bryan Cranston', $person->name ?? '');
    }
    ...
    {
        // Create a client with an invalid base URI to test error handling
        $invalidClient = TVMazeClient::create('Test-Client/1.0');
        
        // This should throw a TVMazeException, not a generic exception
        $this->expectException(TVMazeException::class);
        $invalidClient->searchShows('test');
    }
}
